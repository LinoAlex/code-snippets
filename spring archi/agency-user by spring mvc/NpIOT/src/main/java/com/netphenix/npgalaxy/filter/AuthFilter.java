package com.netphenix.npgalaxy.filter;
...
public class AuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            String dashienceDomain = filterConfig.getInitParameter("npiot-domain");
            ...
            System.out.println("the session----->"+session.getId());
            if (session != null) {
                if (subDomain != null && !subDomain.isEmpty()) {
                    session.setAttribute("agencyPath", subDomain);
                }
                if (session.getAttribute("isAuthenticated") != null
                        && (boolean) session.getAttribute("isAuthenticated")
                        && session.getAttribute("username") != null) {
                    String agencyPath = session.getAttribute("agencyPath") + "";
