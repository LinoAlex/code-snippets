 ...
 */
package com.netphenix.npgalaxy.admin.dao;

import com.netphenix.npgalaxy.dao.BaseDao;
...
//        query.setParameter("userName", username);
    ...
    public VbUser findUser(String agencyPath, String username, Agency agency) {
        if (agency == null || username == null || username.isEmpty()) {
            return null;
        }
        Query query = sessionFactory.getCurrentSession().createQuery("from VbUser where agencyId = :agency and agencyId.agencyPath = :path and userName = :userName");//.getNamedQuery("VbUser.findByUserName");
        query.setParameter("userName", username);
        query.setParameter("agency", agency);
        query.setParameter("path", agencyPath);
        List<VbUser> users = query.list();
    ...
    public Agency findAgencyByPath(String agencyPath) {
        if (agencyPath == null || agencyPath.isEmpty()) {
            return null;
        }
        Query query = sessionFactory.getCurrentSession().createQuery("from Agency where agencyPath = :agencyPath");
        query.setParameter("agencyPath", agencyPath);
